    ...
    <div className="min-h-screen bg-white font-sans text-slate-800 antialiased">
      ...
      <nav className="sticky top-0 z-50 bg-white border-b border-slate-100 px-6 py-4">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          {/* Nav Links - Only About */}
          <div className="hidden md:flex items-center space-x-8">
            <a href="#" className="font-bold text-[#FF5E14] border-b-2 border-[#FF5E14] pb-1 text-sm uppercase tracking-widest">About</a>
          </div>

          {/* Right Side Icons & Call Info */}
      ...
      <section className="relative h-[650px] flex items-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 w-full text-white">
          <div className="max-w-2xl space-y-8">
            ...
            <button className="bg-[#FF5E14] hover:bg-[#e65512] text-white px-10 py-5 rounded font-black text-xs uppercase tracking-[0.2em] transition-all flex items-center group shadow-xl shadow-orange-600/20">
              Start Journey Today
        ...
        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2 z-20 w-full max-w-7xl px-6 hidden lg:grid grid-cols-3 gap-8">
          {[
            { title: 'Safe & Secure', icon: <ShieldCheck className="w-9 h-9"/>, border: '[#FF5E14]' },
            { title: 'Peer Mentorship', icon: <Users className="w-9 h-9"/>, border: '[#0A1128]' },
            { title: 'Mental Guidance', icon: <Target className="w-9 h-9"/>, border: '[#FF5E14]' }
          ].map((card, i) => (
            <div key={i} className={`bg-white p-12 rounded-xl shadow-2xl flex items-start space-x-6 border-b-[6px] border-${card.border} hover:-translate-y-3 transition-transform duration-300`}>
              <div className="p-4 bg-slate-50 rounded-lg text-[#FF5E14]">
                {card.icon}
              </div>
              <div>
                <h3 className="font-black text-2xl text-[#0A1128] tracking-tight">{card.title}</h3>
                <p className="text-sm text-slate-500 mt-2 font-medium italic">Empowering your personal growth through safe support.</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="py-40 mt-20 max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-24 items-center">
        <div className="relative">
          <div className="rounded-[3rem] overflow-hidden shadow-2xl relative">
            ...
            <div className="absolute bottom-10 left-10 bg-[#FF5E14] text-white p-10 rounded-[2.5rem] shadow-2xl max-w-[280px]">
              ...
              <p className="text-[11px] font-black uppercase tracking-[0.2em] opacity-90 leading-tight">Women Empowered Worldwide</p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-10">
          ...
          <p className="text-slate-600 text-lg leading-relaxed italic">
            Humara maksad aapko ek safe space dena hai jahan aap bina kisi darr ke baat kar sakein. Humari community aapke growth ke liye dedicated hai.
          </p>
          ...
          <button className="flex items-center text-[#0A1128] font-black uppercase text-xs tracking-[0.3em] border-b-4 border-[#0A1128] pb-2 pt-4 hover:text-[#FF5E14] hover:border-[#FF5E14] transition-all">
      ...
      <section className="py-32 bg-slate-50">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-3 gap-12">
            {[
              { title: 'Mental Health', img: 'https://images.unsplash.com/photo-1544367567-0f2fcb009e0b?auto=format&fit=crop&q=80&w=400', icon: <Heart/> },
              { title: 'Group Chat', img: 'https://images.unsplash.com/photo-1529156069898-49953e39b3ac?auto=format&fit=crop&q=80&w=400', icon: <MessageSquare/> },
              { title: 'Career Path', img: 'https://images.unsplash.com/photo-1517245386807-bb43f82c33c4?auto=format&fit=crop&q=80&w=400', icon: <Target/> }
            ].map((item, idx) => (
              <div key={idx} className="bg-white rounded-[2rem] overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-500 group">
                ...
                <div className="p-10 space-y-6">
                  ...
                  <p className="text-slate-500 text-sm leading-relaxed italic">Hum situation analyze karke resources aur community support provide karte hain.</p>
                  <button className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 group-hover:text-[#FF5E14] flex items-center transition-colors">
      ...
      {/* Final Contact/Consultation Form */}
      <section className="mx-6 mb-24 grid lg:grid-cols-2 rounded-[4rem] overflow-hidden shadow-2xl">
        ...
        <div className="bg-[#0A1128] p-16 lg:p-24 space-y-12">
          ...
          <form className="grid grid-cols-1 md:grid-cols-2 gap-6" onSubmit={(e) => e.preventDefault()}>
            <input type="text" placeholder="Your Name" className="bg-white/5 border border-white/10 rounded-xl p-5 text-white placeholder:text-slate-500 focus:border-[#FF5E14] outline-none transition-all font-bold text-xs uppercase tracking-widest" />
            <input type="email" placeholder="Email ID" className="bg-white/5 border border-white/10 rounded-xl p-5 text-white placeholder:text-slate-500 focus:border-[#FF5E14] outline-none transition-all font-bold text-xs uppercase tracking-widest" />
            <textarea placeholder="How can we help?" className="md:col-span-2 bg-white/5 border border-white/10 rounded-xl p-5 h-44 text-white placeholder:text-slate-500 focus:border-[#FF5E14] outline-none transition-all font-bold text-xs uppercase tracking-widest resize-none"></textarea>
